...
public:
	...
	BYTE* CMyImage::RGBto8BitGray(BYTE* sz8Bit, BYTE* sz24Bit)
	{
		int gray;
		for(int j = 0; j < nHeight; j++)
		{
			...
			{
				gray = (sz24Bit[j * nWidthByte + i] * 0.0722)
					+ (sz24Bit[j * nWidthByte + i+1] * 0.7152)
					+ (sz24Bit[j * nWidthByte + i+2] * 0.2126);	// B G R
				sz8Bit[j * nWidth + (i / 3)] = (BYTE)gray;
			}
		}
		return sz8Bit;
	}
